...
void MyRender::setup() {
  ...
  reflectProgram->loadFiles("../ej9-3/reflect2");
  ...
  skyboxProgram.loadFiles("../recursos/shaders/skybox");
  skyboxProgram.compile();
  GLint envmapLoc = skyboxProgram.getUniformLocation("cubeMap");

  if (!envmap.loadDDS("../recursos/imagenes/uffizi.dds"))
    throw std::runtime_error(
      "No se han podido cargar el mapa cúbico de uffizi.dds");

  if (!diffusemap.loadDDS("../recursos/imagenes/uffizi-diffuse.dds"))
    throw std::runtime_error(
      "No se han podido cargar el mapa cúbico de uffizi-diffuse.dds");

  if (!specularmap.loadDDS("../recursos/imagenes/uffizi-phong-100.dds"))
    throw std::runtime_error(
