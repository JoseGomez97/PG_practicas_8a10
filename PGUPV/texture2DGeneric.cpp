...
void saveUnboundSubTexture(const std::string &filename, GLint texId, uint32_t xoffset, uint32_t yoffset, uint32_t width, uint32_t height, uint32_t bpp) {
	...
	switch (bpp) {
	case 8:
		glGetTextureSubImage(texId, 0, xoffset, yoffset, 0, width, height, 1, GL_RED, GL_UNSIGNED_BYTE, bufferSize, bytes.get());
		break;
	case 24:
		glGetTextureSubImage(texId, 0, xoffset, yoffset, 0, width, height, 1, GL_RGB, GL_UNSIGNED_BYTE, bufferSize, bytes.get());
		break;
	case 32:
		glGetTextureSubImage(texId, 0, xoffset, yoffset, 0, width, height, 1, GL_RGBA, GL_UNSIGNED_BYTE, bufferSize, bytes.get());
		break;
	}
...
void Texture2DGeneric::save(const std::string &filename, uint32_t bpp, 
	...
	saveUnboundSubTexture(filename, _texId, xoffset, yoffset, width, height, bpp);
}
